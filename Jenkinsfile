            ...
            steps {
                withCredentials([usernamePassword(
                    ...
                    sh '''
                        ...
                        sed -i "/^backend:/,/^[^ ]/ s|^    repository: .*|    repository: ${IMAGE_REPOSITORY}|" helm-repo/${HELM_VALUES_FILE}
                        sed -i "/^backend:/,/^[^ ]/ s|^    tag: .*|    tag: \"${IMAGE_TAG}\"|" helm-repo/${HELM_VALUES_FILE}
                        ...
                        git commit -m "ci(backend): update image to ${IMAGE_TAG}"
